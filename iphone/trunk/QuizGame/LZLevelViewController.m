...
-(void)selectedLZCell:(NSIndexPath *)LZCellIndexPath
{
    ...
    if (lockstate == 1)
    {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:NSLocalizedString(@"Group Locked", @"") message:NSLocalizedString(@"Please pass the preceding group to unlock this group.", @"") delegate:nil cancelButtonTitle:NSLocalizedString(@"Ok", @"") otherButtonTitles:nil];
        [alert show];
    }
    else
    {
